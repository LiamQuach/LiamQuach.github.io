...
remove_outliers <- function(df, ..., sd_thresh = 3) {
  # Capture the variables to check
  vars <- rlang::enquos(...)
  
  # Filter out non-numeric variables and keep track of which ones were removed
  numeric_vars <- df |>
    select(!!!vars) |>
    select_if(is.numeric) |>
    colnames()
  
  non_numeric_vars <- setdiff(names(select(df, !!!vars)), numeric_vars)
  if (length(non_numeric_vars) > 0) {
    warning(paste("The following variables are not numeric and were ignored:", 
                paste(non_numeric_vars, collapse = ", ")))
  }
  
  # For each numeric variable, calculate z-score and filter rows
  df |>
    mutate(across(all_of(numeric_vars), 
                 ~ (. - mean(., na.rm = TRUE)) / sd(., na.rm = TRUE),
                 .names = "{.col}_z")) |>
    filter(if_all(ends_with("_z"), ~ abs(.) <= sd_thresh)) |>
    select(-ends_with("_z"))  # Remove the z-score columns
}
```
...
impute_missing <- function(df, ..., impute_fun = mean) {
  ...
  # For each numeric variable, impute missing values
  df |>
    mutate(across(all_of(numeric_vars), 
...
fit_model <- function(df, mod_formula, remove_outliers = FALSE, impute_missing = FALSE, ...) {
  # Capture the variables to process
  vars <- rlang::enquos(...)
  
  # Process the data based on the flags
  if (remove_outliers) {
    df <- remove_outliers(df, !!!vars)
  }
  
  if (impute_missing) {
    df <- impute_missing(df, !!!vars)
  }
...
model_results <- diamonds_arg_combos |>
  mutate(model = pmap(
    ...
    function(mod_formula, impute_missing, remove_outliers) {
      fit_model(
        df = diamonds,
        mod_formula = mod_formula,
        remove_outliers = remove_outliers,
